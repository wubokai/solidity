...
/// Upgraded for Day23: consumes router/oracle via getPrice(asset).
contract MiniLendingMC_BadDebt_TWAP is ReentrancyGuard {
    error NotListed();
    error AlreadyListed();
    error ZeroAmount();
    ...
    function listCollateral(address token, bool list) external {
        if (list) {
            if (isCollateralListed[token]) revert AlreadyListed();
            isCollateralListed[token] = true;
            collateralList.push(token);
        } else {
            isCollateralListed[token] = false;
        }
    }
    ...
    function healthFactor(address user) public view returns (uint256) {
        ...
        uint256 adjusted = _mulDivDown(cv, LIQ_THRESHOLD, WAD);
        return _mulDivDown(adjusted, WAD, dv);
    }
    ...
    function _computeRepayAndSeize(
        ...
        uint256 colBal,
        address collateralToken
    ) internal view returns (uint256 actualRepay, uint256 actualSeize) {
        uint256 pAsset = oracle.getPrice(asset);
        uint256 pCol = oracle.getPrice(collateralToken);
    ...
    function _burnDebtShares(address borrower, uint256 repayAmount) internal {
        ...
        if (sharesToBurn > borrowerShares) sharesToBurn = borrowerShares;

        debtSharesOf[borrower] = borrowerShares - sharesToBurn;
        totalDebtShares -= sharesToBurn;
    }
